...
| `S3_DISABLE_CHECKSUMS`   | Disable automatic checksums (required for Cloudflare R2) | No | `false` |
...
Cloudflare R2 offers zero-egress-fee object storage with global edge network integration.
...
S3_BUCKET_NAME=your-bucket-name
S3_FORCE_PATH_STYLE=false
S3_DISABLE_CHECKSUMS=true  # Required for R2 compatibility
```

**Important notes:**

- **Must set `S3_DISABLE_CHECKSUMS=true`** - R2 has limited checksum support and will return `400 Bad Request` without this setting
- R2 uses `auto` as the region value
- Your endpoint includes your account ID: `https://your-account-id.r2.cloudflarestorage.com`
- R2 only supports CRC32C checksums (not CRC32), so Palmr disables checksums when this flag is set

**Benefits:**

- Zero egress fees regardless of data transfer volume
- Global edge network for low latency
- S3-compatible API for easy migration
- Integration with Cloudflare's CDN and Workers
...
**Checksum errors (400 Bad Request):**

If you're using Cloudflare R2 and encountering `400 Bad Request` errors during uploads, this is likely due to incompatible checksum algorithms. R2 has limited checksum support and only accepts CRC32C checksums.

**Solution:**
Set `S3_DISABLE_CHECKSUMS=true` in your environment variables:

```bash
S3_DISABLE_CHECKSUMS=true
```

This disables automatic checksum calculation, which resolves compatibility issues with R2 and other S3-compatible providers with limited checksum support.
