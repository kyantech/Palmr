...
### Solution 2: Docker User Directive (More Secure)

For enhanced security, run the container as a non-root user:
...
**Important**: When using the `user` directive, ensure your bind mount directories have the correct ownership:
...
See the [UID/GID Configuration](/docs/3.2-beta/uid-gid-configuration#running-as-non-root-user) guide for more details.
